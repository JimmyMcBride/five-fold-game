...
Use this file as a Brain-managed project context entrypoint for `FiveFoldGame`.
...
- [game-rules-provenance.md](./docs/game-rules-provenance.md)
- [pocketbase.md](./docs/pocketbase.md)
...
5. Retrieve project memory with `brain find fivefoldgame` or `brain search "fivefoldgame <task>"` when the compiled packet is not enough.
...
- This is a browser dungeon crawler, not the multiplayer table/GM product in the source repository.
- `src/lib/game/` is a pure deterministic TypeScript boundary. No Svelte, HTTP, browser persistence, or PocketBase imports.
- Canonical rule lookup starts in `docs/game-rules/`; adaptations belong in `docs/adaptations/` or approved Plan specs.
- Use the seven project skills under `skills/fivefold-*`. Every implementation ends with `fivefold-testing`, then `fivefold-review`.
- PocketBase creates one client per SvelteKit request. Raw auth and privileged data stay server-side; persisted runs eventually use snapshots, append-only history, and expected-version protection.
- Obsidian + Bone is official design direction. Read `DESIGN.md` and `.impeccable.md` before UI work.
